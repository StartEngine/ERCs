...
* **`function transferFrom(address from, address to, uint256 amount) external returns (bool)`**:
  * **MUST `revert`** if caller is not the RTA (with `OwnableUnauthorizedAccount` error per ERC-6093)
...
- `OwnableUnauthorizedAccount(address)` - When non-RTA attempts restricted operations

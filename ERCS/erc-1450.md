...
interface IERC1450 is IERC20 {
     ...
     * @param amount Number of tokens to transfer
     * @dev Transfers msg.value as fee to RTA. Transfer executes only if:
     *      1. Correct fee is paid
     ...
     */
    function requestTransferWithFee(address from, address to, uint256 amount) external payable returns (uint256 requestId);
     ...
     * @return feeAmount Required fee in wei
     * @return feeToken Address of fee token (address(0) for ETH)
     ...
     * @param feeToken Token to collect fees in (address(0) for ETH)
